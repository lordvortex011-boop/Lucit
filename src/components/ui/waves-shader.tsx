...
// Lucit-tuned: bright, low-contrast — white / pale silver / soft grey
const LUCIT_COLORS: [number, number, number][] = [
  [1.0, 1.0, 1.0],
  [0.965, 0.968, 0.972],
  [0.925, 0.932, 0.94],
  [0.885, 0.895, 0.908],
...
export function ShaderBackground({ className, opacity = 0.55 }: { className?: string; opacity?: number }) {
  ...
  useEffect(() => {
    ...
    const gl0 = canvas.getContext("webgl", { antialias: false })
    if (!gl0) { setNoWebGL(true); return }
    const gl = gl0 as WebGLRenderingContext
    ...
    const resizeCanvas = () => {
      ...
      const rawW = Math.max(1, Math.round(bounds.width * dpr)), rawH = Math.max(1, Math.round(bounds.height * dpr))
    ...
    const updatePointerTarget = () => {
      ...
      const nx = ((pointerClientX - bounds.left) / bounds.width) * 2 - 1, ny = -(((pointerClientY - bounds.top) / bounds.height) * 2 - 1)
  ...
  if (noWebGL) {
    return <div aria-hidden className={className} style={{ width: "100%", height: "100%", background: "linear-gradient(180deg, #FFFFFF 0%, #F8F9FA 55%, #F2F3F5 100%)" }} />
  }

  // reduced-motion: static pale fallback (no canvas)
  // We keep hook early-return above, so this branch renders without WebGL init
  return (
    <>
      <canvas ref={canvasRef} data-shader="waves" className={className} style={{ display: "block", width: "100%", height: "100%", opacity }} />
      <style>{`@media (prefers-reduced-motion: reduce) { canvas[data-shader="waves"] { display: none !important; } }`}</style>
    </>
  )
}
